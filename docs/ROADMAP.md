...
- **Fillet/chamfer showcase (exploratory)** — Boolean-composition approach for known-edge bodies; two new showcase scenes (`chamfered-box`, `filleted-slab`) with design exploration. See [specs/2026-05-29-fillet-chamfer-decomposition-design.md](superpowers/specs/2026-05-29-fillet-chamfer-decomposition-design.md).
- **MCP server** — `apps/mcp` exposing the DAG pipeline as MCP tool calls: library management, document reading, mutations, Lua definitions, exports (STL/SVG/DXF/PNG), cache control, viewer URL management. See [specs/2026-05-29-yacad-mcp-design.md](superpowers/specs/2026-05-29-yacad-mcp-design.md).
- **Lua sub-DAG inspection** — Engine caches the resolved `NodeDoc` for Lua expansions alongside geometry; worker protocol gains `getExpandedDoc`; tree view renders derived nodes with expansion affordance; inspector shows read-only mode for generated nodes. See [specs/2026-05-29-lua-subdag-inspection-design.md](superpowers/specs/2026-05-29-lua-subdag-inspection-design.md).
- **Viewport toolbar** — Display mode cycle (solid / wireframe / solid+edges), camera presets (front / back / left / right / top / bottom / isometric), perspective toggle, zoom controls.
