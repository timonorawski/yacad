...
**Goal:** Ship a stdio MCP server that holds the authoritative yacad `DocSession`, exposes 23 tools (full editing + Lua authoring + export), and serves a freshly-built studio2 as a read-only viewer over HTTP+WS.
...
export interface Ctx {
  ...
  /** Undefined when running with --no-viewer. */
...
## Task 14: Tool registry + MCP stdio plumbing
...
async function main(): Promise<void> {
  ...
  // HTTP+WS is set up by Task 15. For Task 14 we just leave a stub:
  if (!flags.noViewer) {
...
Expected: a JSON-RPC response listing all 23 tools.

- [ ] **Step 5: Commit**

```bash
git add apps/mcp/src/tools/index.ts apps/mcp/src/server.ts
git commit -m "feat(mcp): stdio server + tool registry (23 tools)"
```

---

## Task 15: HTTP+WS server + broadcaster
...
export async function startHttpServer(opts: HttpServerOptions): Promise<HttpServerHandle> {
  ...
  await new Promise<void>((resolveListen) => http.listen(opts.port, resolveListen));
  const url = `http://localhost:${opts.port}/?backend=remote&ws=ws://localhost:${opts.port}/ws`;
  return {
    vfsServer,
    url,
    async close() {
...
```ts
  if (!flags.noViewer) {
    const handle = await startHttpServer({ port: flags.port, libraryDir: flags.libraryDir });
    ctx.vfsServer = handle.vfsServer;
    process.stderr.write(`[yacad-mcp] viewer at ${handle.url}\n`);
  } else {
    process.stderr.write(`[yacad-mcp] running headless (--no-viewer)\n`);
  }
```
...
- [ ] **Step 5: Smoke test (no automated test for HTTP yet — covered by e2e in Task 18)**

```bash
node apps/mcp/dist/server.js --port 0 --no-viewer &
# (the e2e test in Task 18 exercises the full path)
kill %1
```
...
## Task 16: Studio2 backend selector
...
## Task 17: Studio2 viewerMode UI hiding
...
## Task 18: run.sh + .mcp.json + README + Playwright e2e
...
beforeAll(async () => {
  ...
  handle = await startHttpServer({ port, libraryDir: dir });
  ctx = { ...rt, sessions: new Map(), currentDocId: undefined, vfsServer: handle.vfsServer };
  await createDoc(ctx, {
    name: 'box',
    initialDoc: { type: 'box', params: { size: [10, 10, 10], center: true } },
  });
